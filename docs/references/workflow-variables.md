...
|----------|---------------------|---------|---------|
| `simpleTag(string)` | `string`| Use this type if there isn't a built in type for the value you need | [custom-metrics](../../examples/argo/custom-metrics.ts)
...
| `simpleTag({ task: new DagTask('dag_task', {}) result: TaskResult.Succeeded })` | `{{dag_task.Succeeded}}`| This is only used in `DagTask.dependsExpression`| [dag-continue-on-fail](../../examples/argo/dag-continue-on-fail.ts)
| `simpleTag({ task: new WorkflowStep('wf_step', {}) result: TaskResult.Failed })`| `{{wf_step.Failed}}`| This is only used in `WorkflowStep.dependsExpression` | No Example
| `simpleTag({ task: new DagTask('dag_task', {}) })` | `{{dag_task}}`| This is only used in `DagTask.dependsExpression`| [dag-coinflip](../../examples/argo/dag-coinflip.ts)
| `simpleTag({ dagTask: new DagTask('dag_task', {}), output: new OutputArtifact('out_art') })` | `{{tasks.dag_task.outputs.artifacts.out_art}}`| | [dag-daemon-task](../../examples/argo/dag-conditional-artifacts.ts)
...
| `simpleTag({ dagTask: new DagTask('dag_task', {}), output: 'id' })`| `{{tasks.dag_task.outputs.id}}` | Useful when using built-in properties | [daemon-nginx](../../examples/argo/daemon-nginx.ts)
