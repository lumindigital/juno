...
There are several challenges with using template refs.
...
A good practice when using Juno is to self contain all of your workflows, and remove any template refs. Doing so makes it much easier to work on a single workflow without breaking other workflows.

## Prefer objects to strings
...
While the global variables are nice, it's a good practice to prefer passing global variables in as an input
